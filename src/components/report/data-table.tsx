...
export function ProductionTable({ rows }: { rows: ManufacturingRecord[] }) {
...
export function ProductionMatrix({ rows }: { rows: ManufacturingRecord[] }) {
  ...
  return <div className="data-table-wrap"><table className="data-table"><thead><tr><th>Line</th><th>Model</th><th>Plan</th><th>Actual</th><th>Achievement</th><th>Pending</th></tr></thead><tbody>
    {matrix.map((row) => <tr key={`${row.line}-${row.model}`}><td>{row.line}</td><td>{row.model}</td><td>{row.plan.toLocaleString()}</td><td>{row.actual.toLocaleString()}</td><td>{row.plan ? `${(row.actual / row.plan * 100).toFixed(1)}%` : "—"}</td><td>{row.pending.toLocaleString()}</td></tr>)}
