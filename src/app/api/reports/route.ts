...
const reportInput = z.object({ name: z.string().trim().min(2).max(120), slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), datasetId: z.string().min(1), description: z.string().max(1_000).default(""), status: z.enum(["draft", "published", "archived"]).default("draft"), filters: z.array(metadataRecord).default([]), pages: z.array(z.object({ id: z.string(), name: z.string().min(1), ordinal: z.number().int(), hidden: z.boolean().optional(), filters: z.array(metadataRecord).default([]), visuals: z.array(metadataRecord) })).min(1) });
