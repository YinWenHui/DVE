...
const reportInput = z.object({ name: z.string().trim().min(2).max(120), slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), datasetId: z.string().min(1), description: z.string().max(1_000).default(""), status: z.enum(["draft", "published", "archived"]).default("draft"), filters: z.array(metadataRecord).default([]), pages: z.array(z.object({ id: z.string(), name: z.string().min(1), ordinal: z.number().int(), hidden: z.boolean().optional(), drillthrough: z.object({ fields: z.array(z.string().min(1).max(128)).min(1).max(8), keepAllFilters: z.boolean().default(true) }).optional(), filters: z.array(metadataRecord).default([]), visuals: z.array(metadataRecord) })).min(1) });
...
export async function POST(request: Request) {
  ...
  const report = { id: randomUUID(), ...parsed.data, minimumRole: "VIEWER" as const, filters: parsed.data.filters as unknown as import("@/types").ReportFilterDefinition[], pages: parsed.data.pages.map((page) => ({ ...page, drillthrough: page.drillthrough as unknown as import("@/types").DrillthroughDefinition | undefined, filters: page.filters as unknown as import("@/types").ReportFilterDefinition[], visuals: page.visuals as unknown as import("@/types").VisualDefinition[] })) };
