...
const input = z.object({ name: z.string().min(2), slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), datasetId: z.string().min(1), description: z.string(), owner: z.string().trim().min(2).max(120), endorsement: z.enum(["promoted", "certified"]).optional(), status: z.enum(["draft", "published", "archived"]), filters: z.array(z.unknown()).default([]), bookmarks: z.array(z.unknown()).default([]), theme: z.record(z.string(), z.unknown()).optional(), formatPresets: z.array(z.unknown()).default([]), pages: z.array(z.unknown()).min(1) });
