...
import type { ReportBookmarkDefinition, ReportPage } from "@/types";

const input = z.object({ name: z.string().min(2), slug: z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), datasetId: z.string().min(1), description: z.string(), status: z.enum(["draft", "published", "archived"]), filters: z.array(z.unknown()).default([]), bookmarks: z.array(z.unknown()).default([]), pages: z.array(z.unknown()).min(1) });
export async function PUT(request: Request, { params }: { params: Promise<{ reportId: string }> }) {
  ...
  Object.assign(report, { ...parsed.data, bookmarks: parsed.data.bookmarks as ReportBookmarkDefinition[], pages: parsed.data.pages as ReportPage[] }); appendAudit("report.update", "report", report.id, auth.displayName, `Updated ${report.name}.`); return Response.json({ report });
}
