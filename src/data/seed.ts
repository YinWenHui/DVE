...
export const seedReports: Report[] = reportNames.map(([name, slug, description], index) => ({
  ...
  endorsement: index === 0 || index === 4 ? "certified" : index < 4 ? "promoted" : undefined,
  owner: index < 4 ? "Manufacturing Intelligence" : index < 8 ? "Operations Analytics" : "Quality Analytics",
  lastModifiedAt: isoMinutesAgo(45 + index * 110),
  usage: { views30d: 1840 - index * 121, uniqueViewers30d: 138 - index * 7, lastViewedAt: isoMinutesAgo(4 + index * 9) },
  theme: {
